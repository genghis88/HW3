  ...
  public void handle(HttpExchange exchange) throws IOException {
    ...
    if(uriPath.equals("/prf")) {
      Vector<ScoredDocument> scoredDocs = ranker.runQuery(processedQuery,
          cgiArgs._numDocs);
      PseudoRelevanceFeedback prf = new PseudoRelevanceFeedback(_indexer);
      Map<String,Double> expandedTerms = prf.getExpandedTerms(scoredDocs,cgiArgs._numTerms);
      StringBuffer response = new StringBuffer();
      for(String term:expandedTerms.keySet()) {
        response.append(term + "\t" + expandedTerms.get(term)+"\n");
      }
